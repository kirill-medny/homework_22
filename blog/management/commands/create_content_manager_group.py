...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        can_add_blogpost = Permission.objects.get(
            codename="add_blogpost", content_type=content_type
        )
        can_change_blogpost = Permission.objects.get(
            codename="change_blogpost", content_type=content_type
        )
        can_delete_blogpost = Permission.objects.get(
            codename="delete_blogpost", content_type=content_type
        )

        # Назначение прав группе
        content_manager_group.permissions.add(
            can_add_blogpost, can_change_blogpost, can_delete_blogpost
        )

        self.stdout.write(
            self.style.SUCCESS(
                "Successfully created content manager group and assigned permissions"
            )
        )
